...
import seg11.ife.entertainment.game_core.GameState;
import seg11.ife.entertainment.game_core.MoveStrategy;
 ...
 */
public class FourConnectBotStrategy implements MoveStrategy {
  ...
  public ArrayList<Position[]> allFourConnects(FourConnectGameBoard board) {
    ...
    for (int x = 0; x < columCount - 3; x++) {
      for (int y = 0; y < rowCount - 3; y++) {
        final Position[] temp =
          {new Position(x, y), new Position(x + 1, y + 1), new Position(x + 2, y + 2),
  ...
  public class HardBotStrategy implements MoveStrategy {
    ...
    private Integer minMax(FourConnectGameBoard board, int depth, boolean isMaximizing) {
      ...
      if ((depth == 0)
          || (fcRules.checkWin(board, playerList[(botPlayerIndex + (isMaximizing ? 1 : 0)) % 2]))
    ...
    /**
     * Macht einen Testzug rückgängig, indem die Zielzelle auf {@link Player#NONE} zurückgesetzt
     * wird.
     *
     * @param board das Spielfeld
     ...
     * die unterste noch freie Zelle (größter y-Wert mit {@link Player#NONE}). Vollständig gefüllte
     * Spalten werden übersprungen. Das Board wird direkt als Parameter verwendet, um die
     * Abhängigkeit vom äußeren Feld {@code fcBoard} zu vermeiden.
     *
